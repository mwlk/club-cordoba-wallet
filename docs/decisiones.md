...
| Decisión | Elección | Justificación |
|---|---|---|
| Comunicación UI → API en Docker | Nginx como reverse proxy (`location /api/` → `http://api:8080` por la red interna de Compose), frontend compilado con `apiUrl: '/api'` (ruta relativa) | La alternativa (hornear `http://localhost:<puerto>/api` en el bundle de Angular) duplica en dos archivos el puerto publicado del `api` en `docker-compose.yml`; si se cambia uno y no el otro, el frontend rompe en runtime sin error de build. Con proxy, el frontend nunca sale de su propio origen — es agnóstico al mapeo de puertos del host. |
| Instalación de dependencias del frontend en Docker | `pnpm install --frozen-lockfile` (vía `corepack`), no `npm install` | El proyecto usa pnpm (`pnpm-lock.yaml`, `pnpm-workspace.yaml`); `npm install` ignora ese lockfile y resuelve el árbol de dependencias de cero, lo que puede instalar versiones distintas a las probadas localmente (riesgo real con Angular 22 y sus peer deps). `--frozen-lockfile` reproduce exactamente el build local. |
| Versión de pnpm en Docker | `"packageManager": "pnpm@11.10.0"` fijado en `package.json` | Sin esto, `corepack enable` descarga la última versión de pnpm disponible (se observó pnpm 12.6.0) en vez de la 11.10.0 usada en desarrollo local. Verificado en la práctica: con la versión sin pinnear el build fallaba de forma distinta a local — riesgo típico de "funciona en mi máquina, no en Docker". |
| Bug: `pnpm install --frozen-lockfile` fallaba con `ERR_PNPM_IGNORED_BUILDS` | `frontend/pnpm-workspace.yaml`, bloque `allowBuilds`: valores cambiados de placeholders de texto (`"set this to true or false"`) a `true` | Causa raíz (verificada corriendo el install real, no supuesta): el bloque `allowBuilds` había quedado a medio completar — probablemente generado por un `pnpm approve-builds` interactivo interrumpido antes de confirmar — con texto literal en vez de booleanos. En modo no-interactivo (como un build de Docker o CI), pnpm no puede preguntar y aborta el install en vez de asumir un default, aunque `onlyBuiltDependencies` ya listara los mismos paquetes (`@parcel/watcher`, `esbuild`, `lmdb`, `msgpackr-extract`, todos con scripts nativos de compilación). Fix: completar `allowBuilds` con `true` explícito para esos 4 paquetes, sin tocar `onlyBuiltDependencies`. |
| Defaults en `docker-compose.yml` (`${VAR:-default}`) | Se mantienen (`devpass`, `dev-secret-key-change-me`, `http://localhost:4200`) | Permiten `docker-compose up --build` sin crear `.env`, cumpliendo igualmente "toda la configuración vía variables de entorno" (4.2.1): el código de la app solo lee env vars, nunca un literal; el default vive en la capa de orquestación y se pisa sin tocar código ni rebuildear. Son valores obvios de placeholder, no aptos para producción real. |
| `ClubCordobaWallet.sln` → `ClubCordobaWallet.slnx` | Migrado con `dotnet sln ClubCordobaWallet.sln migrate` (.NET 10, `dotnet --version` 10.0.111), `.sln` viejo borrado | Formato nuevo de solution (XML, sin GUIDs) soportado nativamente desde .NET SDK 9.0.200+; más legible y liviano que el `.sln` clásico. Verificado tras migrar: `dotnet build ClubCordobaWallet.slnx` compila los 7 proyectos sin errores. Sin referencias al nombre del `.sln` en Dockerfiles/scripts/CI (`grep` no encontró coincidencias), no hizo falta actualizar nada más. |
| Bug: `api` crasheaba al conectar a Postgres (`DllNotFoundException: libgssapi_krb5.so.2`) | `backend/Dockerfile`, etapa runtime: `apt-get install -y libgssapi-krb5-2` antes de copiar la app publicada | Causa raíz (verificada reproduciendo el error real con volumen limpio, no supuesta): Npgsql negocia GSS encryption contra Postgres 18 y necesita esa lib nativa vía P/Invoke; la imagen base `mcr.microsoft.com/dotnet/aspnet:10.0` no la trae instalada. Sin ella, la conexión fallaba antes de poder correr `db.Database.Migrate()`, dejando la API arriba pero sin poder emitir ni listar credenciales. Verificado el fix con `docker compose down -v && up` de cero: migración se aplica y UC01/UC02 funcionan. |
