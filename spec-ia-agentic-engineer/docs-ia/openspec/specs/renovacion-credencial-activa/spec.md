# Spec: renovacion-credencial-activa

> Spec estable. Origen: `changes/renovacion-credencial-activa` (archivado).
> Mejora propuesta, fuera del alcance obligatorio de `init.txt`. Modifica el
> comportamiento de alta (`alta-credencial`) — ver nota MODIFIED en
...
- **THEN** la respuesta es `200 OK` con `data: { credentialId, validUntil }` de esa credencial (si hay más de una activa, la de `validUntil` más lejano)
...
- **THEN** el comportamiento es el mismo que el alta estándar (sin cambios): se emite y persiste la nueva credencial, sin tocar credenciales anteriores
...
`active` (este campo no se modifica al expirar).
